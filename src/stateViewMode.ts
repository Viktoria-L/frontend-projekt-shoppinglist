...
export function viewMode({
  ...
  let bottomButton = document.querySelector("#newListBtn") as HTMLButtonElement;
  ...
  selectedList.itemList.forEach((item) => {
    ...
    const labelA = document.createElement("label") as HTMLLabelElement;
  ...
  headerName.innerHTML = `
    ...
    <h1 class="nameinput list-color-header-${
    ...
    `;
  headerName.classList.add(
    `list-color-header-${selectedList.color ?? "default"}`
  );
  ...
  async function updateCheckedState(
